<?php

namespace Slimbug\Controllers\Bug;

use Slimbug\Models\Bug;
use Slimbug\Transformers\BugTransformer;
use Interop\Container\ContainerInterface;
use League\Fractal\Resource\Item;
use Slim\Http\Request;
use Slim\Http\Response;

// TODO turn this into a 'working-on' type of signal instead
class FavoriteController
{

    /** @var \Slimbug\Services\Auth\Auth */
    protected $auth;
    /** @var \League\Fractal\Manager */
    protected $fractal;

    /**
     * UserController constructor.
     *
     * @param \Interop\Container\ContainerInterface $container
     *
     * @internal param $auth
     */
    public function __construct(ContainerInterface $container)
    {
        $this->auth = $container->get('auth');
        $this->fractal = $container->get('fractal');
    }

    /**
     * Create a new bug's favourite
     *
     * @param \Slim\Http\Request  $request
     * @param \Slim\Http\Response $response
     *
     * @param array               $args
     *
     * @return \Slim\Http\Response
     */
    public function store(Request $request, Response $response, array $args)
    {
        $bug = Bug::query()->where('slug', $args['slug'])->firstOrFail();
        $requestUser = $this->auth->requestUser($request);

        if (is_null($requestUser)) {
            return $response->withJson([], 401);
        }

        $requestUser->favouriteBugs()->syncWithoutDetaching($bug->id);

        $data = $this->fractal->createData(new Item(
            $bug, 
            new BugTransformer($requestUser->id)
        ))->toArray();

        return $response->withJson(['bug' => $data]);

    }

    /**
     * Delete A Favorite
     *
     * @param \Slim\Http\Request  $request
     * @param \Slim\Http\Response $response
     * @param array               $args
     *
     * @return \Slim\Http\Response
     */
    public function destroy(Request $request, Response $response, array $args)
    {
        $bug = Bug::query()->where('slug', $args['slug'])->firstOrFail();
        $requestUser = $this->auth->requestUser($request);

        if (is_null($requestUser)) {
            return $response->withJson([], 401);
        }

        $requestUser->favouriteBugs()->detach($bug->id);

        $data = $this->fractal->createData(new Item(
            $bug, 
            new BugTransformer($requestUser->id)
        ))->toArray();

        return $response->withJson(['bug' => $data]);
    }

}
